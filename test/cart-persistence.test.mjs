...
test('cart persists across restarts', async () => {
  const cartPath = path.join(process.cwd(), 'test', `cart-persist-${Date.now()}.json`)
  const env = { ...process.env, PORT: '0', CART_FILE: cartPath }
  ...
  const add = await fetch(`http://127.0.0.1:${port1}/api/cart`, { method: 'POST', headers: {'content-type':'application/json'}, body: JSON.stringify({ id: 'persist-1', name: 'Persist Item', price: 1 }) })
  ...
  const found = cartJson.items.find(i => i.id === 'persist-1')
